  ...
  methods: {
    delProduct() {
      ...
      switch (this.delName) {
        ...
        case '優惠券': {
          url = `${process.env.VUE_APP_APIPATH}/api/${process.env.VUE_APP_UUID}/admin/ec/coupon/${this.tempProduct.id}`;
          break;
        }
        case '圖片': {
          url = `${process.env.VUE_APP_APIPATH}/api/${process.env.VUE_APP_UUID}/admin/storage/${this.tempProduct.id}`;
          break;
        }
        default:
          break;
